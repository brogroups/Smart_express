...
module.exports.listFirma = async (req, res) => {
    try {
        const firms = await FirmaModel.find().populate('saleProducts history')
...
module.exports.updateFirma = async (req, res) => {
    try {
        const { id, firma, manager, phone, email, moneyType, address, amount } =
            req.body
        ...
        const updatedFirma = await FirmaModel.updateOne({
            ...
            paymentMethod: moneyType || isFirma.paymentMethod,
            address: address || isFirma.address,
            amount: amount || isFirma.amount,
        })
