...
module.exports.updateOrder = async (req, res) => {
    try {
        ...
        if (order.status == 'Заказ оплачен') {
            ...
            const workerHistoryPromises = workerRefs.map(async (worker) => {
                return await WorkerHistory.create({
                    workerName: worker.name,
                    productName: order.productName, // assuming you want to store the product name from the order
                    summa: order.price, // assuming you want to store the price from the order
                    productId: order._id, // assuming you want to store the order ID
                    workerId: worker._id,
                })
            })

            await Promise.all(workerHistoryPromises)
