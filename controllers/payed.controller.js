...
module.exports.createPayment = async (req, res) => {
    try {
        const { firmaId, amount, paymentDate, paymentMethod, comment } =
            req.body

        if (!firmaId || !amount || !paymentDate || !paymentMethod || comment) {
...
module.exports.deletePayment = async (req, res) => {
    try {
        ...
        await FirmaModel.updateOne(
            { _id: payment.firmaId },
            {
                $pull: { payments: payment.id },
                $inc: { debts: +payment.amount },
            }
        )
